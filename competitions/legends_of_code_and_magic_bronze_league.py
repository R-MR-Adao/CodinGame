...
while True:
    ...
    if phase == 'draft':
        ...
        guards   = {card:i for i,card in enumerate(my.hand) if card.abilities['G']} # search guards
        items    = {card:i for i,card in enumerate(my.hand) if card.card_type > 0}  # search items
        if len(items) > 0:
